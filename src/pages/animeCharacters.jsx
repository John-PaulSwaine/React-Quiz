        ...
        <div>
            ...
            <div className="questions">
                ...
                <div className="buttons" style={{ display: 'flex', justifyContent: 'space-between', width: '300px' }}>
                    ...
                    <button className={`incorrect ${isIncorrectClicked.kirito ? 'incorrect-button-clicked' : ''}`} onClick={() => handleIncorrectClick('kirito')} disabled={disabled.kirito}>Kirika</button>
                    <button className={`incorrect ${isIncorrectClicked.kirito ? 'incorrect-button-clicked' : ''}`} onClick={() => handleIncorrectClick('kirito')} disabled={disabled.kirito}>Yuki</button>
                </div>
                <p className={`info ${infoClass.kirito}`} style={{ display: isCorrectClick.kirito || isIncorrectClicked.kirito ? 'block' : 'none' }}>Kirito, the protagonist of Sword Art Online, is a skilled swordsman known for his exceptional combat abilities, resilience, and strong sense of justice, often characterized by his iconic black attire and dual-wielding swordsmanship.</p>
            </div>
            <div className="questions">
                ...
                <div className="buttons" style={{ display: 'flex', justifyContent: 'space-between', width: '300px' }}>
                    ...
                    <button className={`incorrect ${isIncorrectClicked.asuna ? 'incorrect-button-clicked' : ''}`} onClick={() => handleIncorrectClick('asuna')} disabled={disabled.asuna}>Hikari</button>
                    <button className={`incorrect ${isIncorrectClicked.asuna ? 'incorrect-button-clicked' : ''}`} onClick={() => handleIncorrectClick('asuna')} disabled={disabled.asuna}>Klein</button>
                </div>
                <p className={`info ${infoClass.asuna}`} style={{ display: isCorrectClick.asuna || isIncorrectClicked.asuna ? 'block' : 'none' }}>Asuna, a central character in Sword Art Online, is a formidable swordswoman and the vice commander of the Knights of the Blood Oath, renowned for her exceptional speed and combat skills, as well as her compassionate and determined nature.</p>
            </div>
            <div className="questions">
                ...
                <div className="buttons" style={{ display: 'flex', justifyContent: 'space-between', width: '300px' }}>
                    ...
                    <button className={`incorrect ${isIncorrectClicked.apdnarg ? 'incorrect-button-clicked' : ''}`} onClick={() => handleIncorrectClick('apdnarg')} disabled={disabled.apdnarg}>Goku</button>
                    <button className={`incorrect ${isIncorrectClicked.apdnarg ? 'incorrect-button-clicked' : ''}`} onClick={() => handleIncorrectClick('apdnarg')} disabled={disabled.apdnarg}>Naruto</button>
                    <button className={`incorrect ${isIncorrectClicked.apdnarg ? 'incorrect-button-clicked' : ''}`} onClick={() => handleIncorrectClick('apdnarg')} disabled={disabled.apdnarg}>Luffy</button>
                </div>
                <p className={`info ${infoClass.apdnarg}`} style={{ display: isCorrectClick.apdnarg || isIncorrectClicked.apdnarg ? 'block' : 'none' }}>Apdnarg Otum, a character from Yu-Gi-Oh, is a skilled duelist who uses a mask to conceal his identity and participates in the Kaiba Corp Grand Championship, where he faces off against renowned duelists like Joey Wheeler.</p>
            </div>
            <div className="questions">
                ...
                <p className={`info ${infoClass.atem}`} style={{ display: isCorrectClick.atem || isIncorrectClicked.atem ? 'block' : 'none' }}>Atem, the ancient Pharaoh from Yu-Gi-Oh, is the spirit residing within the Millennium Puzzle, known for his exceptional dueling skills and his role as Yugi Muto's alter ego in battles against formidable opponents.</p>
            </div>
            <div className="questions">
                ...
                <p className={`info ${infoClass.bell}`} style={{ display: isCorrectClick.bell || isIncorrectClicked.bell ? 'block' : 'none' }}>Bell Cranel, the protagonist of DanMachi, is a determined and aspiring adventurer known for his unique ability to grow stronger rapidly and his desire to prove himself as a hero while seeking to protect those he cares about.</p>
            </div>
            <div className="questions">
                ...
                <p className={`info ${infoClass.chris}`} style={{ display: isCorrectClick.chris || isIncorrectClicked.chris ? 'block' : 'none' }}>Chris, a character from Konosuba, is a cunning and agile thief who often acts with a laid-back demeanor, using her skills to navigate the challenges of a fantasy world while forming friendships with the main protaginists.</p>
            </div>
            <div className="questions">
                ...
                <div className="buttons" style={{ display: 'flex', justifyContent: 'space-between', width: '300px' }}>
                    ...
                    <button className={`incorrect ${isIncorrectClicked.gowther ? 'incorrect-button-clicked' : ''}`} onClick={() => handleIncorrectClick('gowther')} disabled={disabled.gowther}>Diane</button>
                </div>
                <p className={`info ${infoClass.gowther}`} style={{ display: isCorrectClick.gowther || isIncorrectClicked.gowther ? 'block' : 'none' }}>Gowther, the Goat Sin of Lust from The Seven Deadly Sins, is a powerful and enigmatic magician known for his ability to manipulate memories and emotions, often embodying a complex mix of innocence and cunning.</p>
            </div>
            <div className="questions">
                ...
                <div className="buttons" style={{ display: 'flex', justifyContent: 'space-between', width: '300px' }}>
                    ...
                    <button className={`incorrect ${isIncorrectClicked.heathcliff ? 'incorrect-button-clicked' : ''}`} onClick={() => handleIncorrectClick('heathcliff')} disabled={disabled.heathcliff}>Kirito</button>
            ...
            <div className="questions">
                ...
                <p className={`info ${infoClass.megumin}`} style={{ display: isCorrectClick.megumin || isIncorrectClicked.megumin ? 'block' : 'none' }}>Megumin, the foremost mage of the Crimson Demons from KonoSuba, is a quirky and passionate young girl who specializes in explosion magic, which she can only cast once per day, making her a humorous yet formidable ally.</p>
            </div>
            <div className="questions">
                ...
                <p className={`info ${infoClass.mokuba}`} style={{ display: isCorrectClick.mokuba || isIncorrectClicked.mokuba ? 'block' : 'none' }}>Mokuba Kaiba, the younger brother of Seto Kaiba in Yu-Gi-Oh!, is a kind-hearted and loyal character who often serves as a moral compass for his brother, showcasing a deep devotion to both family and friends.</p>
            </div>
        </div>
    );
};
